...
const policy = {
  ...
  "proxy-groups": [
    ...
    },
    {
      "name": "📌 指定节点",
      "type": "select",
      "filter": "(?i)^(?!.*(?:(?:\\d+(\\.\\d*)?\\s*GB|traffic|expire|premium|github|isp|流量|官网|网址|官址|机场|套餐|应急|时间|重置|剩余|[到过]期|订阅|失联|下载|可用|建议)))(?=[\\s\\S]*(?:(?=.*(Oracle|香港|台湾|新加坡|美国|日本|韩国|Hong Kong|Taiwan|Singapore|USA|Japan|Korea))))[\\s\\S]*$",
      "include-all": true,
      "proxies": [
        "🌍 其他地区"
      ]
