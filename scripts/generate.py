#!/usr/bin/env python3
"""Generate client configurations from one shared policy."""
import argparse
import copy
import json
import re
...
HEADER = '# Generated by scripts/generate.py; edit source.yaml or list/*.list.\n'
...
def render_js(policy, src):
    ...
    js = '// Generated from source.yaml; Clash Party / FlClash override.\n'
...
def simplify_config(common, src):
    """Map business policies to four groups; preserve rule contents and order."""
...
def compile_config(src, offline=False):
    ...
    files['output/override.js'] = js  # Keep the legacy import URL working.
    ...
    # INI creates node groups; base YAML supplies native providers.
    # Pass RULE-SET references through, including ASN and text domain/ipcidr providers.
...
def main():
    ...
    parser.add_argument('--offline', action='store_true', help='Use committed remote snapshots; read local lists directly')
    parser.add_argument('--check', action='store_true', help='Check for outdated outputs and stale snapshots without writing')
    ...
    if args.check:
        ...
        # Fetch and validate everything before writing outputs.
