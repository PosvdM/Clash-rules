...
修改前检查当前分支、工作区和远端最新提交，保留用户及 Actions 机器人的修改。`files/`、`others-rules/`、`others-yml/` 等历史目录不属于当前生成流程，不要顺手清理。

生成产物包括 `output/`、`rules/main.ini` 和 `yml/GeneralClashConfig.yml`。修改生成器或配置源后重新生成；不要单独修改产物。需要删除的过期拆分快照由生成器清理。
