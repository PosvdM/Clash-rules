...
供 Clash Party、FlClash 和 Stash 使用的分流配置。通过一份 `source.yaml` 管理规则引用、策略组和 DNS，自定义规则存放在 `list/`。
...
同一订阅只启用一份主规则覆写；客户端额外设置的 DNS 覆盖若与本配置冲突，需要关闭。TUN 的开关、协议栈和路由由客户端管理，本配置不指定。
...
新增列表、调整规则引用顺序或策略组、修改 DNS 时，编辑 [source.yaml](source.yaml)。`output/`、`rules/main.ini` 和 `yml/GeneralClashConfig.yml` 由程序生成，无需手动维护。
