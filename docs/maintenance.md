...
国旗放在机场图标和 `|` / `｜` 分隔符之后；没有这类前缀时放在名称开头。上一版的 `🇮🇳 🌸|印度…` 会调整成 `🌸|🇮🇳 印度…`，其他已有国旗的名称保持不变。无法识别、同时命中多个地区、会与已有名称冲突时不改名；较长的完整名称优先，避免将“印度尼西亚”误认成“印度”。原来的图标、编号和线路说明不删除。节点连接参数和凭据保持不变，并同步更新已加载节点的 `dialer-proxy`、provider 的 `proxy` 及 `override.dialer-proxy` 引用。
